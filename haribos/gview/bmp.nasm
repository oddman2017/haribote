...
/* エラ〖コ〖ドを手す。とりあえず润0にしてるだけ */
envは64KBのワ〖クエリアである。黎片8dwは手り猛脱になっている。
带券拉らしく, インスタンス恃眶弄には蝗えないようだ。JPEG_initで
base_imgを侯るのはdecodeのときだけでいいと蛔う。
%endif
...
.endif:
	...
	mov edx, eax
	add eax, byte 3			;size<1GBを簿年してedxを痰?
	...
	pop edx
	add esi, eax
	add esi, edx			;呵姜乖の呵稿にはpuddingがないと斧るべき
...
.bpp24:
	...
	.do24.1:
	  ...
	  .do24.2:
	    ...
	    mov ax, [esi]
	     add esi, byte 3
	    ;负咖借妄 eax=24bitColor, edx=work, ecx=counter, ebx=work
	    ;李圭さんの捡蹋で俗ったル〖チンを积ってくるもよし(ぉ
	    ;疙汗橙欢ル〖チンを积ってくるもよし
